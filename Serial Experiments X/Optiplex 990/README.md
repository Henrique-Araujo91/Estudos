...
O software mais importante era justamente o sistema operacional. Dois sistemas operacionais, no caso. Windows 7 já era capaz de rodar de forma nativa em SSD's, portanto foi escolhido um SSD Sata para servir como moradia para esse sistema. Windows XP, no entando, predatava essa tecnologia. Algumas tecnicas podem ser usadas para combinar o Windows XP com um SSD; a mais obvia é chamada de *superprovisionamento*, onde cerca de 20% do espaço do SSD é deixado sem particionamento, de forma que o controlador do SSD possa fazer o serviço de otimização, sem depender da ajuda das funções que normalmente estariam presentes no SO (Trim).