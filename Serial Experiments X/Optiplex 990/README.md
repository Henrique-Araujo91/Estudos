...
Mas o que isso significava na prática? Era isso que eu estava disposto a testar com o Windows XP ligando em uma placa de vídeo, que foi produzida sem drivers para esse sistema operacional. O primeiro passo já havia dado; a placa, com sua entrada HDMI, já era útil para ligar o computador. A parte de ligar o computador com a GPU usando os drivers VBE era significativa porque evitava de ativar os drivers de vídeo onboarding da CPU. E como eu já havia dito, também não possuia monitores funcionais para essa tarefa. 

O que mais, além de ligar o computador, os drivers básicos seriam capazes de oferecer? Para ter a resposta para essa questão, é importante entender o que acontece em nível software nessa situação; quando o Windows XP percebe que não possui o arquivo .sys (o tradutor nativo) para conversar com os milhares de CUDA Cores da arquitetura Pascal da GPU instalada, ele não entra em pânico, e sim aciona um protocolo de emergência chamado Fallback, revertendo a placa mãe ao estado de um cérebro primordial, como o de réptil. O Windows carrega um driver jurássico e universal chamado vgapnp.sys (Adaptador de Vídeo Padrão). Esse driver ignora 99% do silício da GPU e pede apenas uma coisa: "Me dê um Framebuffer". A placa de vídeo está atuando apenas como uma "tela em branco" na memória (o framebuffer). Ela não processa absolutamente nenhum gráfico, ela apenas exibe passivamente a matriz de pixels que o Windows manda para ela.

A GPU nessa situação atua como uma tela em branco na memória. Ela não processa nenhum gráfico, apenas exibe passivamente a matriz de pixels que o Windows manda para ela. Sem a aceleração por hardware (DirectX/OpenGL) da GPU, essa função recai toda sobre a CPU. É o que é conhecido como Software Rendering.

O gemini me aconselhou um teste prático para ver os efeitos práticos dessa situação; abrir o gerenciador de tarefa do Windows XP (Ctrl + Shift + Esc) e deixar na aba desempenho. Logo depois, abrir uma janela qualqeur e movimentar ela rapidamente em circulos ao redor da área de trabalho. Duas coisas aconteceram nesse momento; o gráfico de uso da CPU subiu rapidamente, e o movimento da janela ao redor da tela sofreu glitchs visuais horríveis - o chamado efeito gelatina. Caso houvesse uma GPU com suporte instalada no sistema, o uso da CPU se manteria em zero, e o movimento da janela seria instantâneo.

Outro detalhe que o Gemini apontou, é que o driver genérico não consegue fazer uso de recursos avançados do cabo de saída de vídeo, prendendo o monitor em resoluções 4:3.

Um detalhe sutil; o abandono do gerenciamento de energia (ACPI); o sistema operacional perde a capacidade de entrar em Standby ou hibernação. O clássico botão amarelo do menu de desligar do Windows XP simplesmente fica cinza, indisponível. Isso ocorre porque a GPU é um dos componentes mais crítico para as regras de energia do sistema.
